...
# podman-remote's ssh:// client (jpodman, the port forwarder) offers the
# host's ssh-agent keys as well as the machine key, and sshd's default of 6
# tries runs out first. The file is only edited when the line is missing;
# the restart below applies it (a reload here would fail on first boot,
# before the host keys exist).
if ! grep -qx 'MaxAuthTries 20' /etc/ssh/sshd_config; then
  sed -i '' -e 's/^#\{0,1\}MaxAuthTries.*/MaxAuthTries 20/' /etc/ssh/sshd_config
  grep -qx 'MaxAuthTries 20' /etc/ssh/sshd_config || echo 'MaxAuthTries 20' >> /etc/ssh/sshd_config
fi
