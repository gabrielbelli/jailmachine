...
# Guest sshd lines jm relies on (internal/cli/sshd.go applies the same set to
# older disks):
...
# - ClientAliveInterval 30, ClientAliveCountMax 4: sshd ends a session whose
#   client has gone (a host that slept, a killed tunnel) within two minutes,
#   so a dead session does not keep an idle machine awake (ADR 0009).
# A line is only edited when it is missing, and the edit is put back if
# "sshd -t" rejects it once the host keys exist; the restart below applies
# it (a reload here would fail on first boot, before the host keys exist).
cp -p /etc/ssh/sshd_config /etc/ssh/sshd_config.jm
for jm_kv in 'MaxAuthTries 20' 'ClientAliveInterval 30' 'ClientAliveCountMax 4'; do
  if ! grep -qx "$jm_kv" /etc/ssh/sshd_config; then
    sed -i '' -e "s/^#\{0,1\}${jm_kv%% *}.*/$jm_kv/" /etc/ssh/sshd_config
    grep -qx "$jm_kv" /etc/ssh/sshd_config || echo "$jm_kv" >> /etc/ssh/sshd_config
  fi
done
...
service sshd keygen
if sshd -t; then
  rm -f /etc/ssh/sshd_config.jm
else
  mv /etc/ssh/sshd_config.jm /etc/ssh/sshd_config
fi
