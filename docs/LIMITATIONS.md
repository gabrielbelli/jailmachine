...
| `/tmp` cannot be shared at its own path | `-v /tmp/x:/app` fails with `ocijail: … source path does not exist: /tmp/x (create the directory first)` — or, if that path happens to exist inside the guest too, silently binds the guest's own empty directory | **ours**, forced by macOS — `/tmp` is a symlink to `/private/tmp`, and a share at the guest's `/tmp` would shadow it | Write `-v /private/tmp/x:/app`. `$TMPDIR` (`/var/folders/...`) is shared by default and needs nothing | — |
...
> is routable from the Mac — publish ports instead. Name resolution is the
> one place jailmachine is measurably *better* than the alternatives.
