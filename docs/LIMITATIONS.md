...
| Wake from each client after the sleeper backoff fix | the e2e suite's `e2e-suspend` machine (4 vCPU / 2048 MiB, official 15.1 image) and the author's machine (4096 MiB, four 9p shares), `jm` built from `d4dcd60` | same Mac | 2026-09-14 |
...
| Wake, timed from the client's request (2026-09-14) | Engine socket `_ping` with no wrapper **0.69 s**; `jpodman ps` **0.77 s** |
| Wake from every client, `jm` `d4dcd60` (2026-09-14) | e2e, 2048 MiB guest: `docker ps` **1.1 s**, `docker version` **1.2 s**, `docker compose ls` **1.4 s**, `podman --connection <name>-sock ps` **1.5 s**, `jpodman ps` **1.9 s**, engine socket `_ping` **2.5 s**, `podman --connection <name> ps` over ssh:// **2.5 s**, `podman run -d -p` **4.1 s** to a started container. The author's 4096 MiB machine with four shares, seven wakes by `docker ps`, `docker compose ls` and `podman --connection <name>-sock ps`: **1.8–4.2 s**. Before the fix, a client arriving after a suspend that followed a held-connection wake waited out a 10 s backoff: 11.4–11.7 s in e2e, 12.15 s on the author's machine |
...
| Unmeasured wake under memory pressure, and an uncalibrated idle CPU threshold | Wakes take 0.7–4.2 s on this Mac (2026-09-14); measure one with most of the guest's memory in use and the Mac swapping; calibrate the idle CPU threshold (25 % of one core today) from an hour of samples of an idle guest | **Us** |
